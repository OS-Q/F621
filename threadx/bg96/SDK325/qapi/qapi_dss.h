...
#define QAPI_DSS_RADIO_TECH_UNKNOWN 0x00000000                 /**< Technology is unknown. */
...
#define QAPI_DSS_CALL_INFO_DEVICE_NAME_MAX_LEN 12  /**< Maxiumum length of the device name. */
...
#define QAPI_DSS_IPSEC_FILTER_MASK_SPI            0x00000001 /**< Security parameter index. @newpage */
...
typedef struct qapi_DSS_IP_Header_Filters_s
{
  ...
  /**< Depending on the IP version set, either the IPv4 or the IPv6 information is valid. Values:\n
      ...
      - QAPI_DSS_IP_VERSION_6 (0x06)   --  IPv6 @vertspace{-14}
   */
...
{
  qapi_DSS_Port_Info_Filter_Mask_t valid_params; /**< Bitmask that denotes which parameters contain valid values. */
...
{
  uint8_t filter_rules_valid;  /**< Set to TRUE if filter rules are being passed. */
...
{
  uint8_t filter_handles_valid;  /**< Set to TRUE if filter handles are being passed. */
  uint32_t filter_handles_len;   /**< Set to the number of elements in the filter handles. */
  uint32_t filter_handles[QAPI_DSS_MAX_EXCEPTION_FILTERS]; /**< List of handles that uniquely identify added filter rules. */
...
static __inline void qapi_dss_cb_uspace_dispatcher(UINT cb_id,
                                                               void *app_cb,
                                                               UINT cb_param1,
                                                               UINT cb_param2,
                                                               UINT cb_param3,
                                                               ...
                                                               UINT cb_param8)
{
  void (*pfn_app_cb1) (qapi_DSS_Hndl_t , void*,qapi_DSS_Net_Evt_t,qapi_DSS_Evt_Payload_t*);

  if (cb_id == DATA_CUSTOM_CB_DSS_SRVC_HNDL)
  {
    qapi_DSS_Evt_Payload_t *evt_payload = (qapi_DSS_Evt_Payload_t *)cb_param4;

    pfn_app_cb1 = (void (*)(qapi_DSS_Hndl_t, void*,qapi_DSS_Net_Evt_t, qapi_DSS_Evt_Payload_t*))app_cb;
   (pfn_app_cb1)((qapi_DSS_Hndl_t)cb_param1, 
                 (void*)cb_param2,
                 (qapi_DSS_Net_Evt_t)cb_param3,
                 (qapi_DSS_Evt_Payload_t *)cb_param4);

    /* Release the allocated resources for the application by the kernel. Don't release the event payload.
       As per the legacy behavior, the resources allocated for the payload is released by the caller. */
    if (evt_payload)
      tx_byte_release((void *)evt_payload); 
  }
}
...
/*
 * Macro that Releases Byte Pool Pointer for DSS Application
 * Parameter 'a' : Handle 
 * On success, QAPI_OK is returned. On error, QAPI_ERROR is returned.
 * (This Macro is only used in DAM Space)
 */

#define qapi_DSS_Release(a)                               dss_destroy_indirection(a, TXM_QAPI_DSS_RELEASE)

/** @} */

#define qapi_DSS_Get_Data_Srvc_Hndl(a,b,c)                ((qapi_Status_t) (_txm_module_system_call12)(TXM_QAPI_DSS_GET_DATA_SRVC_HNDL         , (ULONG) a, (ULONG) b, (ULONG) c, (ULONG) qapi_dss_cb_uspace_dispatcher, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0, (ULONG) 0))
...
/*
 * Macro that Passes Byte Pool Pointer for DSS Application
 * Parameter 'a' : Handle
...
#define qapi_DSS_Pass_Pool_Ptr(a,b)                        dss_set_byte_pool(a,b)

/** @} */

static __inline int dss_set_byte_pool(qapi_DSS_Hndl_t hndl, void *dss_byte_pool_ptr)
{
  int ret = QAPI_ERROR;
  qapi_cb_params_uspace_ptr_t *uspace_memory_dss = NULL;
  
  /* Retrieve the user space information stored internally */
  qapi_data_map_handle_to_u_addr(hndl, QAPI_APP_DSS, (void**)&uspace_memory_dss);
  /* Handle to user space information already exists */
  if (uspace_memory_dss)
    return QAPI_OK;
  
  tx_byte_allocate(dss_byte_pool_ptr, (VOID **) &uspace_memory_dss, sizeof(qapi_cb_params_uspace_ptr_t), TX_NO_WAIT);
  if (NULL == uspace_memory_dss)
    return QAPI_ERROR;
  
  memset(uspace_memory_dss, 0x00, sizeof(qapi_cb_params_uspace_ptr_t));

  /* Initialize user space information */
  uspace_memory_dss->usr_pool_ptr = dss_byte_pool_ptr;  

  /* Map the user space information to the handle internally and store */ 
  ret = qapi_data_map_u_addr_to_handle((void *)(hndl), QAPI_APP_DSS, uspace_memory_dss, 0x00);
  if (ret != QAPI_OK)
  {
    /* Release the allocated resources */
    if (uspace_memory_dss)
      tx_byte_release(uspace_memory_dss);
  }

  return ret;
}

static __inline int dss_destroy_indirection(qapi_DSS_Hndl_t hndl, int val)
{
  int ret = QAPI_ERROR;
  qapi_cb_params_uspace_ptr_t *uspace_memory_dss = NULL;
...
/** @ingroup qapi_DSS_Get_Current_Data_Bearer_Tech
    ...
    @param[in] hndl Handle received from qapi_DSS_Get_Data_Srvc_Hndl().
    @param[in] bearer_tech  Pointer to where to retrieve the data bearer technology.
...
/** @ingroup qapi_DSS_Nipd_Send
    ...
    Sends non-IP UL data. In the DL, non-IP data received by the DSS module
    is passed to the application using the registered application callback.

    @param[in] hndl Handle received from qapi_DSS_Get_Data_Srvc_Hndl().
    @param[in] data Non-IP data payload buffer that is to be sent.
