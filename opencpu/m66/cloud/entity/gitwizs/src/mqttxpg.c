...
int MQTT_SenData( pgcontext pgc, char *szDID, ppacket pbuf,/*uint8 *buf,*/ int buflen )
{
    ...
    u32 sn = pgc->rtinfo.stChannelAttrs.cloudClient.sn;
...
int Mqtt_DispatchPublishPacket( pgcontext pgc,unsigned char *packetBuffer,int packetLen )
{
    ...
    unsigned char*pTemp;
    unsigned short cmd;
    u32 sn,sn_temp1,sn_temp2,sn_temp3,sn_temp4;
    unsigned short *pcmd=NULL;
    ...
    if(Adapter_Strncmp((const char*)topic,"app2dev/",Adapter_Strlen("app2dev/"))==0)
    {
        ...
        if( cmd==0x0093 )
        {
            sn_temp1  = (u32)(pHiP0Data[4+varlen+1 + sizeof(cmd)]);
            sn_temp2  = (u32)(pHiP0Data[4+varlen+2 + sizeof(cmd)]);            
            sn_temp3  = (u32)(pHiP0Data[4+varlen+3 + sizeof(cmd)]);
            sn_temp4  = (u32)(pHiP0Data[4+varlen+4 + sizeof(cmd)]);

            sn = (sn_temp4 << 24)|(sn_temp3 << 16)|(sn_temp2 << 8)|sn_temp1;
            
            //APP_DEBUG("p1 =%d\r\n",pHiP0Data[4+varlen+1 + sizeof(cmd)] );
            
            //APP_DEBUG("p2 =%d\r\n",pHiP0Data[4+varlen+2 + sizeof(cmd)] );

            //APP_DEBUG("p3 =%d\r\n",pHiP0Data[4+varlen+3 + sizeof(cmd)] );

            //APP_DEBUG("p4 =%d\r\n",pHiP0Data[4+varlen+4 + sizeof(cmd)] );
           // sn = *((u32 *)(&(pHiP0Data[4+varlen+1 + sizeof(cmd)])));
