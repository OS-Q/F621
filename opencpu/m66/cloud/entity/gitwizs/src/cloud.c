...
u32 Cloud_ConfigDataHandle( pgcontext pgc /*int32 cloudstatus*/ )
{
    ...
    //@modify  remove network check here
    ...
    switch( cloudstatus )
    {
       ...
       case CLOUD_REQ_GET_GSERVER_TIME:
            ...
            if(RET_SUCCESS != ret)
            {
                ...
                APP_DEBUG("Get Gserver time failed!\r\n");
                pGlobalVar->rtinfo.clock = 0;
            ...
            else
            {
                ...
                GAgent_Get_Gserver_Time( &pGlobalVar->rtinfo.clock, pCloudConfiRxbuf,200 );
            }
            
            pgc->rtinfo.waninfo.CloudStatus = CLOUD_CONFIG_OK;
            
            Adapter_SendMsg(g_mcu_task_id,MSG_ID_GSERVER_TIME_RESPONSE,NULL,NULL);
            break;
        default :
            break;
    }
