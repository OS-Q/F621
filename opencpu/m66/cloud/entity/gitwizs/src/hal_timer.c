...
//MOUDLE status update handler
static void Timer_Module_Staus_Handler(u32 timerId, void* param);
...
void HAL_Timer_Init(s32 timerid,pgcontext pgc)
{
    ...
    if(HAL_MODULE_STATUS_TIMER == timerid)
    {
        ret = Ql_Timer_Register(HAL_MODULE_STATUS_TIMER, Timer_Module_Staus_Handler, (void *)pgc);
    }
    if(ret <0)
    {
...
static void Timer_MQTT_Connect_Handler(u32 timerId, void* param)
{
    ...
    if(g_mqtt_connect_count >= 10)
    {
        if(!g_mqtt_connect_status)
        {
           ...
           Ql_GPRS_DeactivateEx(MQTT_CONTEXT,TRUE);
           Adapter_SendMsg(g_cloud_task_id,MSG_ID_GPRS_STATUS_CHECK,NULL,NULL);
        }
    }
    else
    {
        ...
        else
        {
          ...
          Adapter_SendMsg(g_cloud_task_id,MSG_ID_MQTT_SOCKET_INIT,NULL,NULL);
        }
    }
}
...
static void Timer_Cloud_HeartBeat_Handler(u32 timerId, void* param)
{
    ...
    if(g_cloud_hearbeat_count >= 2)
    {
        ...
        Adapter_TimerStop(HAL_CLOUD_HEATBEAT_TIMER);
...
static void Timer_Module_Staus_Handler(u32 timerId, void* param)
{
    ...
    Adapter_SendMsg(g_mcu_task_id,MSG_ID_MODULE_STATUS_UPDATE,NULL,NULL);
}
